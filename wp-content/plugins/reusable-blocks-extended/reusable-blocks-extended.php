 ...
 * @param WP_Post $post
 */
function reblex_enable_gutenberg_post( $can_edit, $post ) {
 ...
 */
function reblex_enable_gutenberg_post_type( $can_edit, $post_type ) {
...
function reblex_reusable_menu_display( $type, $args ) {
	...
	$args->_builtin = false;
	$args->labels->name = esc_html__( 'Reusable Blocks', 'reusable-blocks-extended' );
	$args->labels->menu_name = esc_html__( 'Reusable Blocks', 'reusable-blocks-extended' );
...
function reblex_reusable_screen_fill_column( $column, $ID ) {
	...
	switch( $column ) {
		case 'reblex-reusable-instances' :
			...
			$instances = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}posts WHERE post_content LIKE '$search' and post_status IN ('publish', 'draft', 'future', 'pending')" );
...
function reblex_display_modal( $ID ) {
	...
	if ( $styles ) {
		...
		?>
		<button type="button" class="button button-secondary reblex_button" id="reblex_button_<?php echo $ID; ?>" data-target="#reblex_div_<?php echo $ID; ?>">
