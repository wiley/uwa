(function($) {
	...
	function toggleSubmenuState(CurrentSubmenu) {
		let currentSubmenuButton = CurrentSubmenu.closest(
				'.menu-item-has-children'
			).find('.sub-menu__toggle'),
			newAriaHiddenState =
				CurrentSubmenu.attr('aria-hidden') === 'true' ? false : true;
		newAriaExandedState =
			currentSubmenuButton.attr('aria-expanded') === 'false' ? true : false;
	...
	function submenuToggleHandler() {
		...
		var hasOpenSubmenu =
			Submenus.filter('[aria-hidden="false"]').length == 1 ? true : false;
		var openingNewSubmenu =
			CurrentSubmenu.attr('aria-hidden') == 'true' ? true : false;
	...
	MainMenu.on('mouseenter focusin', '.sub-menu', function(event) {
